...
CREATE TABLE recepcion_producto (
    ...
    orden_pedido_producto_id  NUMBER NOT NULL
);
...
ALTER TABLE hora
    ADD CONSTRAINT hora_persona_fkv2 FOREIGN KEY ( medico_persona_id )
        REFERENCES persona ( id );
...
ALTER TABLE recepcion_producto
    ADD CONSTRAINT recepcion_producto_orden_pedido_producto_fk FOREIGN KEY ( orden_pedido_producto_id )
        REFERENCES orden_pedido_producto ( id );
...
CREATE SEQUENCE seq_orden_pedido_producto START WITH 1 INCREMENT BY 1;
...
INSERT INTO producto (id, nombre, precio_venta, stock, stock_minimo, estado, familia_producto_id) VALUES (seq_producto.nextval, 'Producto 3', 1000, 0, 10, '1', 3);
INSERT INTO producto (id, nombre, precio_venta, stock, stock_minimo, estado, familia_producto_id) VALUES (seq_producto.nextval, 'Producto 4', 1000, 0, 10, '1', 1);
...
create or replace NONEDITIONABLE PROCEDURE insertar_servicio (
    nombre  VARCHAR2,
    newid   OUT NUMBER
) IS
BEGIN
    newid := seq_servicio.nextval;
...
create or replace NONEDITIONABLE PROCEDURE modificar_servicio (
    old_id                  NUMBER,
    new_nombre                 VARCHAR2
...
create or replace NONEDITIONABLE PROCEDURE insertar_orden_pedido (
    persona_id number,
    proveedor_id number,
    newid   OUT NUMBER
) IS
BEGIN
    newid := seq_orden_pedido.nextval;
...
create or replace NONEDITIONABLE PROCEDURE insertar_orden_pedido_producto (
...
BEGIN
    newId := seq_orden_pedido_producto.nextval;
    INSERT INTO orden_pedido_producto (
        id,
        precio_unitario,
        cantidad,
        orden_pedido_id,
...
create or replace NONEDITIONABLE PROCEDURE insertar_recepcion (
    persona_id number,
    proveedor_id number,
    newId OUT NUMBER
) IS
BEGIN
    newId := seq_recepcion.nextval;
...
create or replace NONEDITIONABLE PROCEDURE insertar_recepcion_producto (
    cantidad number,
    fecha_vencimiento varchar2,
    recepcion_id number,
    orden_pedido_producto_id number
) IS
BEGIN
...
create or replace NONEDITIONABLE PROCEDURE agregar_stock (
    cantidad number,
    producto_id number
) IS
BEGIN
